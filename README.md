...
A Spring Boot application using Apache Camel to perform SFTP (and local) file transfers with a **token file pattern**. Files are only picked up when a corresponding token file is present at the source, and a token file is generated at the destination after each successful transfer.
...
| `sftp.token.file.extension` | `.done` | Extension appended to the data file name to form the token file name (e.g., `data.csv` → `data.csv.done`). |
...
The application follows a token file pattern on both the source and destination sides:
...
3. Create the source directory and place a data file with its token file:
   ```bash
...
5. The logs will show the file being picked up, written, and the token file created:
   ```
...
6. Verify the destination directory contains both `sample.csv` and `sample.csv.done`.
